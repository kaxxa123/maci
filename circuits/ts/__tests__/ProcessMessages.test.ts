...
import { MaciState, Poll, packProcessMessageSmallVals, MESSAGE_TREE_ARITY, STATE_TREE_ARITY } from "maci-core";
import { hash2, hash5, IncrementalQuinTree } from "maci-crypto";
...
describe("ProcessMessage circuit", function test() {
  ...
  describe("4 users, 2 messages", () => {
    ...
    it("should produce the correct state root and ballot root", async () => {
      ...
      const inputs = poll.processMessages(pollId, true) as unknown as IProcessMessagesInputs;
  ...
  describe("23 users, 23 messages", () => {
    ...
    before(() => {
      // Sign up and publish
      const users = new Array(2).fill(0).map(() => new Keypair());
      ...
      // send 23 valid messages
      users.forEach((userKeypair) => {
        const index = users.indexOf(userKeypair) + 1;
        const command = new PCommand(
          BigInt(index),
          userKeypair.pubKey,
          voteOptionIndex, // voteOptionIndex,
    ...
    it("should generate proofs of message processing", async () => {
  ...
  describe("2 users, 1 message", () => {
    ...
    it("should correctly process messages", async () => {
      const inputs = poll.processMessages(pollId) as unknown as IProcessMessagesInputs;
      // Calculate the witness
      const witness = await circuit.calculateWitness(inputs);
      await circuit.expectConstraintPass(witness);
  ...
  describe("1 user, 2 messages", () => {
    ...
    it("should produce the correct state root and ballot root", async () => {
      ...
      const emptyBallot = new Ballot(poll.maxValues.maxVoteOptions, poll.treeDepths.voteOptionTreeDepth);
      const emptyBallotHash = emptyBallot.hash();
      const ballotTree = new IncrementalQuinTree(STATE_TREE_DEPTH, emptyBallot.hash(), MESSAGE_TREE_ARITY, hash5);
